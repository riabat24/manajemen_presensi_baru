...
import com.rplbo.app.demo.models.*;
...
public class DashboardAdminController implements Initializable {
    ...
    @FXML public void handleSimpan(ActionEvent event) {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            if (isEditMode) {
                ...
                pst.setString(1, nama); pst.setString(2, nik); pst.setInt(3, Integer.parseInt(txtJabatan)); pst.setInt(4, Integer.parseInt(txtDept)); pst.setString(5, email); pst.setString(6, status); pst.setInt(7, selectedIdKaryawan);
                pst.executeUpdate();
                showAlert(Alert.AlertType.INFORMATION, "Sukses", "Data berhasil diperbarui!");
            } else {
                // Skrip untuk INSERT data baru
                // Tambahkan Statement.RETURN_GENERATED_KEYS untuk mengambil ID karyawan yang baru saja dibuat
                PreparedStatement pst = conn.prepareStatement("INSERT INTO karyawan (nama, nik, id_jabatan, id_departemen, email, status, tanggal_masuk, no_telepon) VALUES (?, ?, ?, ?, ?, ?, CURRENT_DATE(), '-')", java.sql.Statement.RETURN_GENERATED_KEYS);
                pst.setString(1, nama); pst.setString(2, nik); pst.setInt(3, Integer.parseInt(txtJabatan)); pst.setInt(4, Integer.parseInt(txtDept)); pst.setString(5, email); pst.setString(6, status);
                pst.executeUpdate();

                // Mengambil ID Karyawan yang baru di-generate oleh database
                ResultSet rs = pst.getGeneratedKeys();
                if (rs.next()) {
                    int newIdKaryawan = rs.getInt(1);

                    // Buat akun otomatis di tabel 'pengguna'
                    // Username menggunakan NIK, Password default '12345'
                    PreparedStatement pstAkun = conn.prepareStatement("INSERT INTO pengguna (id_karyawan, username, password, role) VALUES (?, ?, ?, 'karyawan')");
                    pstAkun.setInt(1, newIdKaryawan);
                    pstAkun.setString(2, nik); // Username pakai NIK agar unik
                    pstAkun.setString(3, "12345"); // Password sementara
                    pstAkun.executeUpdate();
                }
                showAlert(Alert.AlertType.INFORMATION, "Sukses", "Karyawan disimpan! Akun otomatis dibuat (Username: " + nik + ", Password: 12345).");
            }
    ...
    @FXML public void handleLogout(ActionEvent event) {
        ...
        try {
            ...
            stage.setScene(new Scene(FXMLLoader.load(getClass().getResource("/views/login-view.fxml"))));
